...
public class KnightSwordD {
	...
	public static void dialogue(Client c, int dialogue, int npcId) {
		...
		case 651:
			...
			break;
		case 652:
			DialogueHandler.sendNpcChat2(c, "Yes, sure one moment.",
				"He MUSTN'T know what happened though!", c.talkingNpc, "Squire");
			c.knightS += 1;
			c.nextChat = 653;
			break;
			/*/
...
/*/
		case 653:
			DialogueHandler.sendStatement(c,
					"The Squire gives you the portrait.");
			c.getItems().addItem(666,1);
			c.knightS += 1;
			c.nextChat = 0;
			break;
		...
		case 666:
			...
			break;
		case 673:
			if(c.getItems().playerHasItem(667, 1)){
				DialogueHandler.sendNpcChat1(c, "You should bring the Squire that sword.", c.talkingNpc, "Thurgo");
				c.nextChat = 0;
			}else{
				DialogueHandler.sendNpcChat1(c, "Did the sword work?", c.talkingNpc, "Thurgo");
				c.nextChat = 674;
			}
			break;
		case 674:
			DialogueHandler.sendPlayerChat2(c, "I've seemed to have lost my sword.",
		...
		case 675:
			if (c.getItems().playerHasItem(2351, 2)
					&& c.getItems().playerHasItem(668, 1)){
			DialogueHandler.sendNpcChat1(c, "Sure, just let me see that blurite, and iron bars.", c.talkingNpc, "Thurgo");
			c.nextChat = 676;
					}else{
						DialogueHandler.sendNpcChat2(c, "Sure, but you need to get more",
							"blurite ore, and iron bars.", c.talkingNpc, "Thurgo");
					c.nextChat = 0;}
						break;
		case 676:
			DialogueHandler.sendStatement(c,
